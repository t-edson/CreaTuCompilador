...
- Cadenas. Ocupan hasta 255 bytes.
Además se soportan arreglos de hasta 255 elementos de estos tipos de datos.

Las expresiones en asignaciones solo pueden tener hasta dos operandos:
    "a := b"
    "a := b + c"

Los registros de trabajo, son los que se usan para devolver el resultado de las
expresiones u operandos. Son dos:
* Los registros EAX y EBX, para devolver valores numéricos.
* Las variables _strA y _strB para devolver valores de cadena.
}
program titan;
uses sysutils;
...
var srcRow     : integer;  //Número de línea áctual
...
//Información sobre variables locales
var nLocs      : integer;     //Número de variables locales.
var nPars      : integer;     //Número de parámetros.
var locNames   : array[0..255] of string;
var locType    : array[0..255] of integer;
//Variable de trabajo
var curLocName : string;
var curLocType : integer;  //Tipo de dato: 1->integer. 2->string.
var isParam    : integer;  //Bandera para indicar si es parámetro.
...
begin
  if eof(inFile) then begin
    if next_is_EOL<>0 then begin exit(1)
...
procedure TrimSpacesLine;
//Extrae espacios o comentarios. Útil cuando se desea buscar un elemento que no debe
//estar en otra línea.
begin
...
begin
  ...
  if srcToken = ':' then begin  //Es declaración de tipo común
    ...
    if typName = 'integer' then begin
      NextToken;  //Toma token
      asmline('LOCAL ' + varName + ': DWORD');
      //Registra variable
      locNames[nLocs] := varName;
      locType[nLocs]  := 1;  //Integer
      inc(nLocs);
    end else if typName = 'string' then begin
      NextToken;  //Toma token
      asmline('LOCAL ' + varName + '[256]: BYTE');
      //Registra variable
      locNames[nLocs] := varName;
      locType[nLocs]  := 2;  //String
      inc(nLocs);
    end else begin
      MsjError := 'Tipo desconocido: ' + typName;
      exit;
...
procedure FindLocal;
// Busca la variable local con el nombre que está en "srcToken", y actualiza las
// variables: "curLocName", "curLocType" y "isParam".
// Si no encuentra, devuelve cadena vacía en "curLocName".
var
  tmp   : string;
  contin: integer;  //Bandera para continuar bucle.
  curVar: integer;  //Índice de variable.
begin
  ...
  //Verifica si encontró
  if contin=0 then begin
    curLocName := locNames[curVar];
    curLocType := locType[curVar];
    if curvar<nPars then begin isParam:=1 end else begin isParam:=0 end;
    exit;  //"curVar" contiene el índice
  end;
  //No encontró
  curLocName := '';
...
procedure ParserProcedure;
//Procesa la declaración de un procedimiento.
var procName, parName, typName: String;
var i: integer;
begin
  NextToken;   //Toma el "procedure"
  TrimSpaces;  //Quita espacios
  if srcToktyp<>2 then begin
    MsjError := 'Se esperaba un identificador.';
    exit;
  ...
  procName := srcToken;
  NextToken;  //Toma nombre de variable
  asmout('  ' + procName + ' PROC ');
  //Procesa parámetros
  Capture('(');
  if MsjError<>'' then begin exit; end;
  TrimSpaces;
  while srcToken <> ')' do begin
  ...
  Capture(')');
  if MsjError<>'' then begin exit; end;
  //Genera lista de parámetros en orden inverso
  i := npars;
  while i>0 do begin
    parName := locNames[i-1];
    asmout(',');
    asmout(parName + ':DWORD');
    i := i - 1;
  end;
  asmline('');    //Salto de línea final.
  //Verifica delimitador de instrucción
  Capture(';');
  if MsjError<>'' then begin exit; end;
  //Aquí debemos procesar las variables locales
  TrimSpaces;
...
begin
  ...
  asmline(mov_eax + 'DWORD PTR [' + vName + '+esi]');
...
begin
  ...
  if srcToktyp = 3 then begin  //Literal Número
    resType := 1;   //Integer
    if srcToken<>'000' then begin
      asmline(mov_eax + srcToken);   //Huevo de pascua para depuración.
    end;
    NextToken;
    ...
    if srcToken = 'length' then begin
      ...
      if MsjError<>'' then begin exit; end;
      if srcToktyp<>2 then begin
        MsjError := 'Se esperaba identificador.';
        exit;
      end;
      //Solo se permite parámetro variable.
      FindVariable;   //Actualiza "curVarName", "curVarType" y "curVarArSiz".
      if curVarName = '' then begin
        MsjError := 'Identificador desconocido: ' + srcToken;
        exit;
      end;
      if curVarType<>2 then begin
      ...
      Capture(')');
      if MsjError<>'' then begin exit; end;
      if regnum=1 then begin  //Devuelve en A
        asmline('invoke szLen, addr ' + curVarName);
      end else begin
        asmline('push eax');   //Salva por si acaso
        asmline('invoke szLen, addr ' + curVarName);
        asmline('mov ebx, eax');  //Devuelve en EBX
        asmline('pop eax');     //Recupera
      end;
      resType := 1;  //Devuelve número
    end else begin
      //Busca variable
      FindVariable;   //Actualiza "curVarName", "curVarType" y "curVarArSiz".
      if curVarName = '' then begin
        //No es una variable global. Podría ser una variable local.
        FindLocal;  //Actualiza "curLocName", "curLocType" e "isParam".
        if curLocName='' then begin
          MsjError := 'Identificador desconocido: ' + srcToken;
          exit;
        end;
        ...
        NextToken;
        TrimSpaces;
        if isParam = 1 then begin  //Es parámetro.
          if curLocType = 1 then begin         //Variable entera
            asmline(mov_eax + curLocName); //Carga en registro
          end else begin           //Variable cadena
            asmline('invoke szCopy, ' + curLocName + addr_strA);  //"curLocName" es la dirección
          end;
          resType := curLocType;   //Tipo del resultado
        end else begin         //Es variable local.
          if curLocType= 1 then begin         //Variable entera
            asmline(mov_eax + curLocName); //Carga en registro
          end else begin                 //Variable cadena
            asmline('invoke szCopy, addr ' + curLocName + addr_strA);
          end;
          resType := curLocType;   //Tipo del resultado
        end;
        exit;
...
procedure OperAdd;
{Realiza la operación "+" sobre los operandos "EAX" y "EBX". Devuelve resultado
...
begin
  ...
  if op1Type = 1 then begin  //Comparación de enteros
    asmline(macro + ' eax ' + operation + ' ebx');
  end else if op1Type = 2 then begin  //Comparación de cadenas
    asmline('invoke lstrcmp, addr _strA, addr _strB');
...
{Evalúa la expresión actual y genera el código necesario. Actualiza variable "resType"}
begin
...
begin
  FindVariable;   //Actualiza "curVarName", "curVarType" y "curVarArSiz".
  if curVarName = '' then begin
    //No es variable global, tal vez sea parámetro o variable local
...
var
  procName: String;
begin
  procName := srcToken;  //Nombre de procedimiento.
  NextToken;
  Capture('(');
  TrimSpaces;
  //asmout('    invoke ' + procName);
  while srcToken <> ')' do begin
    //Debe seguir un parámetro
    EvaluateExpression;
    if MsjError<>'' then exit;
    if resType=1 then begin   //Entero
      asmline('push eax');  //Salva valor en la pila
    end else begin  //Cadena
      asmline('mov eax, offset _strA');
      asmline('push eax');
    end;
    TrimSpaces;
    //Verifica si hay coma.
  ...
  Capture(')');
  if MsjError<>'' then begin exit; end;
  asmline('call ' + procName);  //Llamada al procedimiento
  asmline('');    //Salto de línea final.
end;
...
begin
  while EndOfBlock<>1 do begin
    if srcToken = 'print' then begin
      ...
      //Es un identificador, puede ser una llamada a función o asignación
      if srcChar = ord('(') then begin  //El caracter después del token actual.
        ProcessCall;
      end else begin     //Debe ser una asignación
        ProcessAssigment;
      end;
      if MsjError<>'' then begin break; end;
    end else begin
...
begin
  ...
  if MsjError='' then begin
    ...
    while srcToken = 'procedure' do begin
      //Es una declaración de procedimiento
      nPars := 0;     //Inicia contador. Se llenarán con parámetros.
      nLocs := 0;     //Inicia contador. Se llenarán con variables locales.
      ParserProcedure;
      if MsjError<>'' then begin break; end;
      //Verifica delimitador de instrucción
      Capture(';');
  ...
  if MsjError='' then begin
    ...
    nPars := 0;     //Inicia contador. No se usa en el prog. principal.
    nLocs := 0;     //Inicia contador. No se usa en el prog. principal.
